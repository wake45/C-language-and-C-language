...
int main(int argc, char argv[]) {
	...
	char *Menu[] = { "1.정보 입력","2.바이오리듬 결과 출력 ","3.그래프 출력","4.스케쥴 입력 ","5.결과 분석","6.종료" }; //menu 포인터 배열
...
void Biorhythm_Analyses(char arr_rythm[][20], char arr_scehdule[][20], char arr[][3], int start , int end) //결과 분석함수
{
	...
	char *physical[] = { "자전거 타기" , "축구 하기" , "달리기" ,  //감성,지성리듬이 좋고 신체리듬이 안 좋을 경우 랜덤으로 값을 정해 이 배열의 그 랜덤값 위치에 있는 행동을 추천함
						 "등산하기" , "농구 하기" , "야구 하기" , 
						 "헬스 하기" , "줄넘기 하기" , "족구 하기" };
	char *emotion[] = { "음악 듣기" , "영화 보기", "카페 가기" ,  //신체.지성리듬이 좋고 감성리듬이 안 좋을 경우 랜덤으로 값을 정해 이 배열의 그 랜덤값 위치에 있는 행동을 추천함
						"산책 하기" , "데이트 하기","술 한잔 하기",
						"여행 가기", "편지 쓰기", "친구들 만나기" };
	char *intellect[] = { "영어 공부하기", "수학 공부하기", "국어 공부하기",   //신체.감성리듬이 좋고 지성리듬이 안 좋을 경우 랜덤으로 값을 정해 이 배열의 그 랜덤값 위치에 있는 행동을 추천함
